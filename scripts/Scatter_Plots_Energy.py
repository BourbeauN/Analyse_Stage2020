...
def main():
    ...
    Data_Filter = ["Energy/20kv_500ns_point-point_water.csv","Energy/5kv_500ns_point-point_water.csv","Energy/20kv_500ns_point-point_heptane.csv","Energy/20kv_100ns_point-point_water.csv"]
   ...
   #To filter through the files in need of filtering and changing file_filter with the TimeStamp_Filter value associated with the filtered infolder
    ...
    if bound == "s" or bound == "b":
        ###PLOTS###
        plt.plot(ET/60, Integration_Fin,'.',markersize = 1, color = 'crimson', linewidth = 0)
        ...
        ###PLOTS###
        plt.plot(ET_file/60, Integration,'.',markersize = 1, color = 'crimson', linewidth = 0)
        plt.xlabel("Elapsed time (minutes)")
        plt.ticklabel_format(axis="y", style="sci")
        plt.ylabel("Energy (J)")
        plt.title("Energy of discharge for {} {} in\n{} with {} configuration".format(tension,pulsewidth,medium,configuration))
