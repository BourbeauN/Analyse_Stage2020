...
def main():
    ...
    pd.DataFrame(PLATEAU, columns = ['Filename','Plateau']).to_csv(os.path.join('Discharge_Delay',"PLATEAU_{}_{}dv_{}dk.csv".format(outfile,args.VOLTAGE_THRESHOLD,args.INDEX_THRESHOLD)))
    pd.DataFrame(VOLT_DIS, columns = ['Filename','Voltage']).to_csv(os.path.join('Discharge_Voltage',"VOLT_DIS_{}.csv".format(outfile)))
