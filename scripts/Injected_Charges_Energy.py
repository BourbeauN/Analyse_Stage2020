...
def main():
    ...
    #info = '_'.join((Pol,Amp,Wid))

    #if args.METHOD == 'all':
    ABS_INJ,REG_INJ,DIS_INJ,NEG_INJ,ABS_ENE = Integration(args.INFOLDER,dk,dv)
    print("finished calculating now saving ... ")
    pd.DataFrame(ABS_INJ, columns = ['Filename','Absolute_Injected_Charges']).to_csv(os.path.join('Audren2/Analysis/IC/{}.csv'.format(Amp)))
    pd.DataFrame(ABS_ENE, columns = ['Filename','Energy']).to_csv(os.path.join('Audren2/Analysis/IE/{}.csv'.format(Amp)))
    pd.DataFrame(REG_INJ, columns = ['Filename','Injected_Charges']).to_csv(os.path.join('Audren2/Analysis/IE/{}_nonabs.csv'.format(Amp)))
